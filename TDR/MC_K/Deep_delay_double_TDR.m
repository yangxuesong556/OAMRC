function [nmse_p,MCK]=Deep_delay_double_TDR(epsilon,TPD,data,FB,...
    gamma,alpha,h,p,...
...
% Training_data0=data(1:T+FB+1)';
% data0=(Training_data0-min(Training_data0))./(max(Training_data0)-min(Training_data0));
%     Input_streaming=data0(1:TPD(1)+1)';
%     Training_data=data0(FB+1:TPD(1)+FB)';
...
y_trained0=y;
OUT=y_trained0(1:M);%测试
XN=data.y(:,Training_steps+1:Training_steps+M)';%参考
XNK=Input_streaming(Training_steps+1:Training_steps+M);%输入
MCK=(min(max(cov(XNK,OUT))))^2/(var(XN)*var(OUT));

Refernce_p=data.y(:,TPD(1)+1:sum(TPD(1:2)));%参考
Test_p=y_trained0';%测试
%nrmse_p=sqrt(mean((Test_p(1:M)-Refernce_p(1:M)).^2));
nmse_p=mean((Test_p(1:M)-Refernce_p(1:M)).^2)/mean((Refernce_p(1:M)-mean(Refernce_p(1:M))).^2);
%nrmse_p=sqrt(mean((Test_p(1:M)-Refernce_p(1:M)).^2)/mean((Refernce_p(1:M)-mean(Refernce_p(1:M))).^2));
% fprintf(' FB=%d De=%d M=%d  h=%f  g=%f  a=%f p=%d  F_nrmse_p=%f \n',FB,Delay_time(1),M,h,gamma,alpha,p,nmse_p)
