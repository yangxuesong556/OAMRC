function [nrmse_p]=Deep_delay_double_TDR(epsilon,TPD,data,FB,...
    gamma,alpha,h,p,...
...
data0=(data-min(data))./(max(data)-min(data));
    Input_streaming=data0(1:sum(TPD(1:2))+1)';
    Training_data=data0(FB+1:sum(TPD(1:2))+FB+1)';
...
y_trained0=y.*(max(data)-min(data))+min(data);

% y_trained0=y;

% Refernce_p=data.y(:,TPD(1)+1:sum(TPD(1:2)));%参考
Refernce_p=Training_data0(:,TPD(1)+1:sum(TPD(1:2)));
