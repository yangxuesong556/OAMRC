...
input_dim=1;
output_dim=1;
n_layers=length(resSize);%返回数组长度
discard=max(floor(TPW(1)/10),TPW(3));%floor 四舍五入
T=sum(TPW(1:2));
%cell为元胞数组函数，可以存储任意的数据类型
%% 初始化
Training_data0=data0(FB+1:T+FB+1);%训练数据
data=(data0-min(data0))./(max(data0)-min(data0)); %归一化
Input_streaming=data(1:T+1); %输入1到3700个数据
Training_data=data(FB+1:T+FB+1); %输出
W_in=cell(1,n_layers);%定义一个1*10的
W=cell(1,n_layers);
...
for i= 1:n_layers
    if i==1
        ...
        [W_in{i},W{i},W_Feedb{i}]=W_Generator(f_Feedb,resSize(i-1)+input_dim,output_dim,resSize(i));%由于从第二层开始输入就是由上一层内部状态以及输入数据的串接，维度为res+inputdim*res
    end
end
%% 训练阶段
...
trainLen=TPW(1);
testLen=TPW(2);
Y=y*(max(data0)-min(data0))+min(data0);%归一化处理
