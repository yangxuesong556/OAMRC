...
TPW=[3200,500,200]; % training sets, testing set, set of washout  3200训练模型，500个预测数据，200冲洗阶段
M=500; % sample of NRMSE
resSize=1000; % number of reservoir node
data0=csvread('D:\MackyG17.csv',1,0); % datasets
tic
for i=1:5
%% ESN
Lr=1;
...
%% DEEP_ESN
La=10;
res=ones(1,La)*round(resSize/La);%ones返回一个全1矩阵
Lr=1;
De=ones(1,La)*0;
...
nrmse_ADRC3(i)=ADRC_main(De3,Lr,f_Feedb,FB,res,M,TPW,data0);%根据Dealy延时的不同对其进行均方差误差的分析
end
